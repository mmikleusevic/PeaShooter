...
namespace Unity.Physics.Editor
{
    ...
    internal class PhysicsShapeAuthoringEditor : BaseEditor
    {
        ...
        private static readonly PhysicsCapsuleBoundsHandle s_Capsule = new()
            { heightAxis = CapsuleBoundsHandle.HeightAxis.Z };
        ...
        private void OnDisable()
        {
            ...
            foreach (PreviewMeshData preview in m_PreviewData.Values)
                preview.Dispose();
        ...
        {
            PhysicsShapeAuthoring shape = target as PhysicsShapeAuthoring;

            float4x4 shapeMatrix = shape.GetShapeToWorldMatrix();
            Bounds bounds = new Bounds();
            switch (shape.ShapeType)
            {
                case ShapeType.Box:
                    BoxGeometry boxGeometry = shape.GetBakedBoxProperties();
                ...
                case ShapeType.Capsule:
                    CapsuleGeometryAuthoring capsuleGeometry = shape.GetBakedCapsuleProperties();
                    float cd = capsuleGeometry.Radius * 2;
                ...
                case ShapeType.Sphere:
                    SphereGeometry sphereGeometry = shape.GetBakedSphereProperties(out EulerAngles orientation);
                    float sd = sphereGeometry.Radius * 2;
                    bounds = new Bounds(sphereGeometry.Center, new float3(sd, sd, sd));
                    break;
                case ShapeType.Cylinder:
                    CylinderGeometry cylinderGeometry = shape.GetBakedCylinderProperties();
                    float cyld = cylinderGeometry.Radius * 2;
                ...
                case ShapeType.Plane:
                    shape.GetPlaneProperties(out float3 center, out float2 size2, out orientation);
                ...
                case ShapeType.Mesh:
                    PreviewMeshData previewData = GetPreviewData(shape);
                    if (previewData != null)
        ...
        private void OnSceneGUI()
        {
            ...
            Color handleColor = shape.enabled ? k_ShapeHandleColor : k_ShapeHandleColorDisabled;
            float4x4 handleMatrix = shape.GetShapeToWorldMatrix();
            using (new Handles.DrawingScope(handleColor, handleMatrix))
            {
                switch (shape.ShapeType)
                {
                    ...
                    case ShapeType.Plane:
                        ...
                        EditorGUI.BeginChangeCheck();
                    {
                        float4x4 m = math.mul(shape.transform.localToWorldMatrix,
                            float4x4.TRS(center, orientation, 1f));
                        using (new Handles.DrawingScope(m))
                        {
                            s_Plane.DrawHandle();
                        }

                        float3 right = math.mul(m, new float4 { x = 1f }).xyz;
                        float3 forward = math.mul(m, new float4 { z = 1f }).xyz;
                        float3 normal = math.cross(math.normalizesafe(forward), math.normalizesafe(right))
                    ...
                    case ShapeType.ConvexHull:
                        if (Event.current.type != EventType.Repaint)
                            break;
                        Vector3[] points = GetPreviewData(shape).Edges;
        ...
        private PreviewMeshData GetPreviewData(PhysicsShapeAuthoring shape)
        {
            ...
            if (!m_PreviewData.TryGetValue(shape, out PreviewMeshData preview))
            {
        ...
        private void UpdateGeometryState()
        {
            ...
            NativeList<float3> skinnedPoints = new NativeList<float3>(8192, Allocator.Temp);
            foreach (PhysicsShapeAuthoring shape in targets)
            {
                // if a custom mesh is assigned, only check it
                using (SerializedObject so = new SerializedObject(shape))
                {
                    Mesh customMesh = so.FindProperty(m_CustomMesh.propertyPath).objectReferenceValue as Mesh;
                    if (customMesh != null)
                    {
                ...
                GeometryState geometryState = GeometryState.Okay;
                using (GetActiveChildrenScope<MeshFilter> scope =
                       new GetActiveChildrenScope<MeshFilter>(shape, shape.transform))
                {
        ...
        private void RecommendedConvexValuesButton()
        {
            ...
            GUIContent buttonLabel = Styles.SetRecommendedConvexValues;
        ...
        private void UpdateStatusMessages()
        {
            ...
            foreach (PhysicsShapeAuthoring shape in targets)
            {
                ...
                {
                    case ShapeType.Box:
                        BoxGeometry box = shape.GetBakedBoxProperties();
                        float max = math.cmax(box.Size);
                        float min = math.cmin(box.Size);
                        if (min < k_Epsilon)
                        {
                        ...
                        break;
                    case ShapeType.Capsule:
                        CapsuleGeometryAuthoring capsule = shape.GetBakedCapsuleProperties();
                        if (math.abs(capsule.Height - 2f * capsule.Radius) < k_Epsilon)
                            m_ShapeSuggestions.Add(Styles.CapsuleSphereSuggestion);
                        break;
                    case ShapeType.Cylinder:
                        CylinderGeometry cylinder = shape.GetBakedCylinderProperties();
            ...
            foreach (string suggestion in m_ShapeSuggestions)
            ...
            {
                float4x4 localToWorld = (t as Component).transform.localToWorldMatrix;
                m_MatrixStates.Add(ManipulatorUtility.GetMatrixState(ref localToWorld));
            }

            m_MatrixStatus =
                StatusMessageUtility.GetMatrixStatusMessage(m_MatrixStates, out string matrixStatusMessage);
        ...
        private void DisplayShapeSelector()
        {
            ...
            foreach (PhysicsShapeAuthoring shape in targets)
            {
                ...
                {
                    case ShapeType.Box:
                        shape.SetBox(shape.GetBoxProperties(out EulerAngles orientation), orientation);
                        break;
                    case ShapeType.Capsule:
                    ...
                    case ShapeType.Cylinder:
                        ...
                        break;
                    case ShapeType.Plane:
                        shape.GetPlaneProperties(out float3 center, out float2 size2D, out orientation);
        ...
        private void AutomaticPrimitiveControls()
        {
            ...
            GUIContent buttonLabel = Styles.GetFitToRenderMeshesLabel(targets.Length, m_MatrixStatus);
        ...
        private void DisplayCylinderControls()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                foreach (PhysicsShapeAuthoring shape in targets)
                {
                    shape.SetCylinder(shape.GetCylinderProperties(out EulerAngles orientation), orientation);
                    EditorUtility.SetDirty(shape);
                }
            }
        ...
        private class PreviewMeshData : IDisposable
        {
            ...
            )
            {
                ...
                switch (shape.ShapeType)
                {
                    ...
                    case ShapeType.Mesh:
                        NativeList<int3> triangles = new NativeList<int3>(1024, Allocator.Temp);
            ...
            {
                using (NativeList<float3> currentPoints = new NativeList<float3>(65535, Allocator.TempJob))
                {
                    uint hash = GetInputHash(
                        shape, currentPoints, m_HashedPoints, m_HashedConvexParameters,
                        out ConvexHullGenerationParameters currentConvexParameters
                    );
                    if (m_InputHash == hash)
                        return;
                ...
                NativeArray<BlobAssetReference<Collider>> output =
                    new NativeArray<BlobAssetReference<Collider>>(1, Allocator.Persistent);
            ...
            private JobHandle ScheduleConvexHullPreview(PhysicsShapeAuthoring shape,
                NativeArray<BlobAssetReference<Collider>> output)
            {
                using NativeList<float3> pointCloud = new NativeList<float3>(65535, Allocator.TempJob);
                ...
                NativeArray<float3> pointsArray = new NativeArray<float3>(
            ...
            private JobHandle ScheduleMeshPreview(PhysicsShapeAuthoring shape,
                NativeArray<BlobAssetReference<Collider>> output)
            {
                using NativeList<float3> points = new NativeList<float3>(1024, Allocator.TempJob);
                NativeList<int3> triangles = new NativeList<int3>(1024, Allocator.Temp);
                ...
                NativeArray<int3> triangleArray = new NativeArray<int3>(
            ...
            {
                bool repaintSceneViews = false;

                foreach (JobHandle job in m_PreviewJobsOutput.Keys.ToArray()) // TODO: don't allocate on heap
                {
                    // repaint scene views to indicate progress if most recent preview job is still in the queue
                    bool mostRecentlyScheduledJob = m_MostRecentlyScheduledJob.Equals(job);
                    repaintSceneViews |= mostRecentlyScheduledJob;

                    if (!job.IsCompleted)
                        continue;

                    NativeArray<BlobAssetReference<Collider>> output = m_PreviewJobsOutput[job];
                        ...
                        else
                        {
                            switch (output[0].Value.Type)
                            {
                                case ColliderType.Convex:
                                    ref ConvexCollider convex = ref output[0].As<ConvexCollider>();
                                ...
                                case ColliderType.Mesh:
                                    ref MeshCollider mesh = ref output[0].As<MeshCollider>();
        ...
        private class FitToRenderMeshesDropDown : EditorWindow
        {
            ...
            private void OnGUI()
            {
                ...
                Rect buttonRect = GUILayoutUtility.GetRect(0f, EditorGUIUtility.singleLineHeight);

                Rect buttonLeft = new Rect(buttonRect)
                {
                    width = 0.5f * (buttonRect.width - EditorGUIUtility.standardVerticalSpacing)
                };
                Rect buttonRight = new Rect(buttonLeft)
                {
            ...
            {
                FitToRenderMeshesDropDown window = CreateInstance<FitToRenderMeshesDropDown>();
