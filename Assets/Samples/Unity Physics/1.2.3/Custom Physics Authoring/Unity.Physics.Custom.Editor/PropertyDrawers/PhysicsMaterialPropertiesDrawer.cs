...
namespace Unity.Physics.Editor
{
    ...
    internal class PhysicsMaterialPropertiesDrawer : BaseDrawer
    {
        ...
        private SerializedProperty GetTemplateValueProperty(SerializedProperty property)
        {
            ...
            Object template = property.FindPropertyRelative("m_Template").objectReferenceValue;
        ...
        {
            SerializedProperty relative = property.FindPropertyRelative(relativePath);
        ...
        {
            SerializedProperty templateValueProperty = GetTemplateValueProperty(property);

            // m_CollisionResponse, collision filter foldout, advanced foldout
            float height = 3f * EditorGUIUtility.singleLineHeight + 2f * EditorGUIUtility.standardVerticalSpacing;

            // m_BelongsTo, m_CollidesWith
            SerializedProperty group = property.FindPropertyRelative(k_CollisionFilterGroupKey);
            if (group.isExpanded)
            ...
            FindToggleAndValueProperties(property, templateValueProperty, "m_CollisionResponse", out _,
                out SerializedProperty collisionResponse);
        ...
        )
        {
            if (templateAssigned)
            {
                ...
                Rect togglePosition = new Rect(position)
        ...
        {
            SerializedProperty template = property.FindPropertyRelative("m_Template");
            bool templateAssigned = template.objectReferenceValue != null;
            SerializedProperty supportsTemplate = property.FindPropertyRelative("m_SupportsTemplate");
            ...
            SerializedProperty templateValue = GetTemplateValueProperty(property);

            FindToggleAndValueProperties(property, templateValue, "m_CollisionResponse",
                out SerializedProperty collisionResponseDropDown, out SerializedProperty collisionResponse);
            ...
            if (collisionResponseEnum == CollisionResponsePolicy.Collide ||
                collisionResponseEnum == CollisionResponsePolicy.CollideRaiseCollisionEvents)
            {
                FindToggleAndValueProperties(property, templateValue, "m_Friction", out toggle,
                    out SerializedProperty friction);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
                DisplayOverridableProperty(position, Content.FrictionLabel, toggle, friction, templateAssigned);

                FindToggleAndValueProperties(property, templateValue, "m_Restitution", out toggle,
                    out SerializedProperty restitution);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
            ...
            SerializedProperty collisionFilterGroup = property.FindPropertyRelative(k_CollisionFilterGroupKey);
            position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
            position.height = EditorGUIUtility.singleLineHeight;
            ...
            if (collisionFilterGroup.isExpanded)
            {
                ...
                FindToggleAndValueProperties(property, templateValue, "m_BelongsToCategories", out toggle,
                    out SerializedProperty belongsTo);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
                ...
                FindToggleAndValueProperties(property, templateValue, "m_CollidesWithCategories", out toggle,
                    out SerializedProperty collidesWith);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
            ...
            // advanced group
            SerializedProperty advancedGroup = property.FindPropertyRelative(k_AdvancedGroupKey);
            position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
            position.height = EditorGUIUtility.singleLineHeight;
            ...
            if (advancedGroup.isExpanded)
            {
                ...
                FindToggleAndValueProperties(property, templateValue, "m_CustomMaterialTags", out toggle,
                    out SerializedProperty customFlags);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
