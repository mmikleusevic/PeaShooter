...
namespace Unity.Physics.Editor
{
    internal abstract class TagsDrawer<T> : PropertyDrawer where T : ScriptableObject, ITagNames
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            int value = 0;
            int everything = 0;
            SerializedProperty sp = GetFirstChildProperty(property);
            ...
            )
            {
                ...
                {
                    int callbackValue = option;
                    menu.AddItem(
                        EditorGUIUtility.TrTextContent(options[option]),
                        ((1 << option) & value) != 0,
                        args =>
                        {
                            KeyValuePair<int, bool> changedBitAndValue = (KeyValuePair<int, bool>)args;
                            sp = GetFirstChildProperty(property);
                ...
                menu.AddItem(
                    ...
                    () =>
                    {
                        ...
                        {
                            string assetPath = AssetDatabase.GenerateUniqueAssetPath($"Assets/{typeof(T).Name}.asset");
