...
public partial struct GridEnemyPositionUpdateSystem : ISystem
{
    ...
    {
        BeginVariableRateSimulationEntityCommandBufferSystem.Singleton ecbSingleton =
            SystemAPI.GetSingleton<BeginVariableRateSimulationEntityCommandBufferSystem.Singleton>();
        EntityCommandBuffer ecb = ecbSingleton.CreateCommandBuffer(state.WorldUnmanaged);

        RefRW<GridComponent> gridComponent = gridEntityQuery.GetSingletonRW<GridComponent>();

        foreach ((RefRW<GridEnemyPositionUpdateComponent> gridEnemyPositionUpdate, Entity entity) in SystemAPI
                     .Query<RefRW<GridEnemyPositionUpdateComponent>>()
                     .WithEntityAccess())
        {
            if (gridEnemyPositionUpdate.ValueRO.status == UpdateStatus.Move)
                MoveEnemyToPosition(gridComponent.ValueRW.enemyPositions, gridEnemyPositionUpdate.ValueRO.oldPosition,
                    gridEnemyPositionUpdate.ValueRO.position, gridEnemyPositionUpdate.ValueRO.entity);
            else
                AddEnemyToPosition(gridComponent.ValueRW.enemyPositions, gridEnemyPositionUpdate.ValueRO.position,
                    gridEnemyPositionUpdate.ValueRO.entity);
    ...
    [BurstCompile]
    public void MoveEnemyToPosition(NativeHashMap<int2, NativeList<Entity>> enemyPositions, int2 oldPosition,
        int2 position, Entity other)
    {
        if (enemyPositions.TryGetValue(position, out NativeList<Entity> list))
